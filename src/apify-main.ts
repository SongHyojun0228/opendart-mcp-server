...
// --- Factory: create a new McpServer per session ---
...
app.post("/mcp", async (req, res) => {
  try {
    ...
    if (!sessionId) {
      ...
      // sessionId is assigned after handleRequest processes the initialize
      if (transport.sessionId) {
        sessions.set(transport.sessionId, { server, transport });
        console.error(`[OpenDART] New session: ${transport.sessionId}`);
      }
      return;
    }

    // Session ID provided but not found
    console.error(`[OpenDART] Session not found: ${sessionId} (active: ${[...sessions.keys()].join(", ")})`);
    res.status(404).json({
      jsonrpc: "2.0",
    ...
    console.error("[OpenDART] POST /mcp error:", err);
    if (!res.headersSent) {
      res.status(500).json({
        jsonrpc: "2.0",
...
app.delete("/mcp", async (req, res) => {
  ...
  if (sessionId && sessions.has(sessionId)) {
    ...
    console.error(`[OpenDART] Session closed: ${sessionId}`);
    return;
  }
  res.status(404).json({
    jsonrpc: "2.0",
